...
const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ project, isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div 
        className={`relative w-full max-w-6xl h-[80vh] bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 transform ${
                  ...
                  <ul className="space-y-2">
                    {project.objetivos.map((objetivo, index) => (
                      <li key={index} className="flex items-start gap-3">
                        ...
                        <span className="text-spark-blue font-inter">{objetivo}</span>
                      </li>
                    ))}
                  </ul>
                </div>
